...
export const handler: Handler = async (event) => {
  ...
  let body: {
    email: string; name?: string; projectId: string; message?: string;
  };
  ...
  // Check for existing portal invite for this project+email, expire it
  ...
  const portalUrl  = `${CONNECT_URL}/portal?token=${invToken}`;
...
<div style="border:1px solid #e5e7eb;border-top:none;padding:32px;border-radius:0 0 12px 12px">
  ...
  <p style="color:#6b7280;margin:0 0 20px;">Your project portal is ready. You can view your project status, review estimates, check invoices, and send us messages directly from the portal.</p>
  ${customNote}
  ...
  try {
    ...
    console.log(`[portal-invite] sent to ${email} for project ${project.name}`);
  } catch (mailErr) {
  ...
  return {
    ...
    body: JSON.stringify({ success: true, invitationId: invitation.id, portalUrl }),
  };
};
