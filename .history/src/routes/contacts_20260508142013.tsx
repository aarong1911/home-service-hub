...
import {
  ...
  Users, UserPlus, Star, Activity, CalendarClock, Loader2,
} from "lucide-react";
...
// ── Phone formatter: 5551234567 → 555-123-4567 ──
      ...
      <Card className="mb-3 p-2.5">
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-[260px] flex-1">
            ...
            <Input placeholder="Search by name, email, or company…" value={search} onChange={(e) => setSearch(e.target.value)} className="h-8 pl-8 text-sm" />
          </div>
                ...
                <tr>
                  <td colSpan={9} className="py-16 text-center">
                    <div className="mx-auto max-w-xs">
                      ...
                      <Button size="sm" className="mt-4"><Plus className="mr-1.5 h-3.5 w-3.5" /> New Contact</Button>
                    </div>
                  </td>
                </tr>
              )}
              ...
              <tbody>
                {colMapping && CONTACT_FIELDS.map((field) => {
                  ...
                  const tagDetection = isTagsField && tagDelimiter === "auto" && mapped >= 0
                    ...
                    : null;
                  const effectiveDelimiter = tagDetection ? tagDetection.delimiter : (isTagsField && tagDelimiter === "auto" ? detectTagDelimiter(previewVals) : tagDelimiter);
                  return (
                    <tr key={field.key} className="border-b border-border last:border-0">
                      <td className="px-3 py-2.5">
                        ...
                        {isTagsField && mapped >= 0 && (
                          <div className="mt-1">
                            <select className="h-6 rounded border border-input bg-transparent px-1.5 text-[11px] focus:outline-none focus:ring-1 focus:ring-ring" value={tagDelimiter} onChange={(e) => setTagDelimiter(e.target.value as TagDelimiter)}>
                      ...
                      <td className="px-3 py-2.5">
                        <Select value={String(mapped)} onValueChange={(v) => setColMapping((prev) => prev ? { ...prev, [field.key]: Number(v) } : prev)}>
                      ...
                      <td className="hidden px-3 py-2.5 sm:table-cell">
                        <span className="line-clamp-1 text-xs text-muted-foreground">
                          {isTagsField && previewVals.length ? previewVals.map((v) => splitTags(v, effectiveDelimiter).map((t) => `[${t}]`).join(" ")).join(", ") : previewVals.length ? previewVals.join(", ") : "—"}
                        </span>
                      </td>
                    </tr>
                  );
          ...
          {importValidation && (
            <div className="space-y-2 border-t border-border pt-3">
              <div className="flex items-center gap-4 text-sm">
                <span className="flex items-center gap-1.5 text-emerald-600"><CheckCircle2 className="h-3.5 w-3.5" />{importValidation.validCount} valid</span>
                {importValidation.errors.length > 0 && (
                  <span className="flex items-center gap-4">
                    <span className="flex items-center gap-1.5 text-amber-600"><AlertTriangle className="h-3.5 w-3.5" />{importValidation.errors.length} will be skipped</span>
                    <Button size="sm" variant="ghost" className="ml-auto h-7 text-xs" onClick={downloadErrorReport}><Download className="mr-1 h-3 w-3" /> Download error report</Button>
                  </span>
                )}
              </div>
              {importValidation.errors.length > 0 && (
                <div className="max-h-32 overflow-y-auto rounded-md border border-border bg-muted/30 p-2">
                  {importValidation.errors.map((err, i) => (
                    <div key={i} className="flex items-start gap-1.5 py-0.5 text-xs text-muted-foreground"><AlertTriangle className="mt-0.5 h-3 w-3 shrink-0 text-amber-500" />{err}</div>
                  ))}
                </div>
              )}
            </div>
...
function FilterChip({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className={`h-8 rounded-md border px-2.5 text-xs font-medium transition-colors ${active ? "border-primary/30 bg-primary-soft text-primary" : "border-border bg-background text-muted-foreground hover:bg-secondary/60"}`}>
      {children}
    </button>
  );
}
...
function Kpi({ label, value, sub, icon: Icon, tone }: { label: string; value: string; sub: string; icon: React.ComponentType<{ className?: string }>; tone: "primary" | "success" | "warning" | "muted" }) {
  const toneClass = { primary: "bg-primary-soft text-primary", success: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400", warning: "bg-amber-500/10 text-amber-600 dark:text-amber-400", muted: "bg-secondary text-muted-foreground" }[tone];
  return (
    <Card className="p-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md ${toneClass}`}><Icon className="h-4 w-4" /></div>
      </div>
    </Card>
  );
}
            ...
            <Tabs defaultValue="overview" className="mt-4">
              <TabsList className="w-full justify-start gap-1 bg-transparent p-0">
                {["overview", "activity", "deals", "projects", "notes", "files"].map((t) => (
                  <TabsTrigger key={t} value={t} className="rounded-md px-3 py-1.5 text-xs capitalize data-[state=active]:bg-secondary data-[state=active]:shadow-none">{t}</TabsTrigger>
                ))}
              </TabsList>
              ...
              <TabsContent value="overview" className="mt-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {contact.tags.map((t) => (<Badge key={t} variant="secondary" className="h-6 rounded text-[11px]">{t}</Badge>))}
                  </div>
                </div>
                <Separator />
              ...
              <TabsContent value="activity" className="mt-4"><ActivityTab contact={contact} /></TabsContent>
              <TabsContent value="deals" className="mt-4"><DealsTab contact={contact} /></TabsContent>
              <TabsContent value="projects" className="mt-4"><ProjectsTab contact={contact} /></TabsContent>
...
// ===== Deals tab (still mock for now) =====

function DealsTab({ contact }: { contact: Contact }) {
  const deals: Deal[] = useMemo(() => mockDeals.filter((d) => d.contactId === contact.id), [contact]);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between rounded-md border border-border bg-secondary/40 px-3 py-2">
        <div className="text-xs text-muted-foreground">
          <span className="font-medium text-foreground">{deals.length}</span> deal{deals.length === 1 ? "" : "s"} · <span className="font-medium text-foreground">{formatMoney(totalValue)}</span> total
        </div>
        ...
        <div key={d.id} className="group rounded-md border border-border bg-card p-3 transition-colors hover:bg-secondary/40">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 text-[11px] text-muted-foreground">Owned by {d.owner} · {d.ageDays}d in stage</div>
            </div>
          ...
          <div className="mt-2 flex items-center justify-between">
            <Badge className={`h-5 rounded px-1.5 text-[10px] font-medium ${stageTone(d.stage)}`} variant="outline">{stageLabel(d.stage)}</Badge>
            <button className="flex items-center gap-1 text-[11px] text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100">Open <ArrowRight className="h-3 w-3" /></button>
          </div>
        </div>
      ))}
    </div>
...
// ===== Projects tab (still mock for now) =====
...
function ProjectsTab({ contact }: { contact: Contact }) {
  ...
  const projects: Project[] = useMemo(
    () => mockProjects.filter((p) => p.client.toLowerCase() === contact.name.toLowerCase() || p.client.toLowerCase().startsWith(first + " ")),
    [contact, first],
  );

  const display = projects.length > 0 ? projects : (() => {
  ...
  if (display.length === 0) return <div className="text-sm text-muted-foreground">No active projects.</div>;
        ...
        <div key={p.id} className="rounded-md border border-border bg-card p-3">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 truncate text-[11px] text-muted-foreground">{p.projectNumber} · {p.type} · {p.address.split(",").slice(-1)[0]?.trim()}</div>
            </div>
            <Badge className={`h-5 rounded px-1.5 text-[10px] font-medium capitalize ${projectStageTone(p.stage)}`} variant="outline">{p.stage.replace("-", " ")}</Badge>
          </div>
          <div className="mt-3 grid grid-cols-3 gap-2 text-[11px]">
            <div><div className="text-muted-foreground">Budget</div><div className="mt-0.5 font-medium tabular-nums">{formatMoney(p.budget)}</div></div>
            <div><div className="text-muted-foreground">Paid</div><div className="mt-0.5 font-medium tabular-nums">{formatMoney(p.paid)}</div></div>
            <div><div className="text-muted-foreground">Progress</div><div className="mt-0.5 font-medium tabular-nums">{p.progress}%</div></div>
          </div>
