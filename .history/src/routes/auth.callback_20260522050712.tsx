...
import { Lock, Eye, EyeOff, Phone, User, MapPin, Shield, Building2, CheckCircle2, Loader2 } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data, error } = await supabase
        .from("invitations")
        .select("id, email, role, first_name, last_name, organization_id, expires_at, status, worker_type")
  ...
  return (
    <AuthLayout title="Set up your account" subtitle="Complete your profile to join your team on RenoMeta Connect.">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2 space-y-1.5">
              ...
              <Input value={addr1} onChange={e => setAddr1(e.target.value)} placeholder="123 Main St" className="h-11 bg-primary-soft/50" autoComplete="street-address" />
            </div>
            <div className="col-span-2 space-y-1.5">
        ...
        <div>
          ...
          {isSub ? (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 space-y-1.5">
                <Label className="text-xs">EIN (Employer Identification Number)</Label>
        ...
        <Button type="submit" disabled={saving} className="h-11 w-full bg-foreground text-background hover:bg-foreground/90">
          {saving ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Setting up…</> : "Create account & join team"}
        </Button>
      </form>
    </AuthLayout>
  );
