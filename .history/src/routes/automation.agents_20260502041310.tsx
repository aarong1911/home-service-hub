...
function AgentsPage() {
  ...
  // Sync URL search param → selected autonomous agent drawer state
  useEffect(() => {
    setSelectedId(urlAgentId ?? null);
  }, [urlAgentId]);

  // Sync URL search param → active top tab.
  // This is what allows /automation/agents?tab=voice to open the Voice Agent tab.
  useEffect(() => {
    const nextTab = urlTab ?? "agents";
    if (nextTab !== topTab) {
      setTopTab(nextTab);
    }
  }, [urlTab, topTab]);
    ...
    <div className="space-y-4">
      ...
      <Tabs value={topTab} onValueChange={handleTopTabChange}>
        <TabsList className="h-9">
          <TabsTrigger value="agents" className="px-4 text-xs">
            Autonomous Agents
          </TabsTrigger>
          <TabsTrigger value="tools" className="px-4 text-xs">
            AI Tools
          </TabsTrigger>
          <TabsTrigger value="voice" className="px-4 text-xs">
            Voice Agent
          </TabsTrigger>
        </TabsList>
