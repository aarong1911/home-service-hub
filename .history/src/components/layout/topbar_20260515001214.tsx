...
async function getOrgId(): Promise<string | null> {
  ...
  const { data: p } = await supabase.from("profiles").select("organization_id").eq("id", session.user.id).maybeSingle();
  if (p?.organization_id) return p.organization_id;
  const { data: m } = await supabase.from("org_memberships").select("org_id").eq("member_id", session.user.id).maybeSingle();
...
export function Topbar() {
  ...
  // ── Logo: direct fetch + realtime (mirrors V5 Header approach) ──────────────
  // Initialized from localStorage so it shows instantly before Supabase responds
  ...
  useEffect(() => {
    ...
    const loadAndSubscribe = async () => {
      ...
      // Direct fetch — always gets the real current value from DB
      ...
      if (data) {
        ...
        const url = raw?.startsWith("blob:") ? null : raw;
        setLogoUrl(url);
        try {
      ...
      // Realtime subscription — updates logo instantly when DB changes
      channelName = `topbar_org_${orgId}`;
      supabase
        .channel(channelName)
        .on(
          "postgres_changes",
          ...
          (payload) => {
            const raw = (payload.new as any).logo_url || null;
            const url = raw?.startsWith("blob:") ? null : raw;
            setLogoUrl(url);
            try {
              if (url) localStorage.setItem(LOGO_KEY, url);
    ...
    // Also re-fetch when org is updated programmatically anywhere in the app
    const onOrgUpdated = () => loadAndSubscribe();
  ...
  const companyInitial = (companyName[0] ?? "R").toUpperCase();
