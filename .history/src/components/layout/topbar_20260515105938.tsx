...
export function Topbar() {
  ...
  useEffect(() => {
    ...
    function applyLogo(raw: string | null | undefined, options?: { fromDB?: boolean }) {
      if (cancelled) return;
    ...
    async function loadLogo() {
      ...
      if (!cancelled && !error) {
        applyLogo(data?.logo_url, { fromDB: true });
      }
      ...
      realtimeChannel = supabase
        ...
        .on(
          ...
          (payload) => {
            applyLogo((payload.new as { logo_url?: string | null })?.logo_url);
          }
        )
        .subscribe();
    }
  ...
  const safeOrgLogo =
  org.logoUrl && !org.logoUrl.startsWith("blob:") && !org.logoUrl.startsWith("data:")
    ? org.logoUrl
    : null;

const displayLogo = logoUrl || safeOrgLogo;
