...
// Vapi Web SDK singleton (dynamic import per Ron's pattern)
declare global {
  interface Window {
    __vapiInstance?: any;
  }
...
export function VoiceAgentTab() {
  ...
  const handleAddAgent = async () => {
    ...
    // Fetch org name for default prompts
    ...
    const defaultSystemPrompt = `You are a friendly and professional receptionist for ${companyName}. Your job is to answer incoming calls, qualify leads by asking about their project type, timeline, and budget range, check the team's calendar for availability, and book appointments. Always be warm, helpful, and concise.`;

    const defaultGreeting = `Hi there! Thanks for calling ${companyName}. My name is Sarah and I'd love to help you get started on your project. Can I ask what type of work you're looking to have done?`;
  ...
  // -- Toggle status (single-active enforcement) --
  const toggleStatus = async (id: string) => {
    ...
    toast.success(
      newActive
        ? `${agent.name} activated — other agents paused`
        : `${agent.name} paused`
    );
    await loadAgents();
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    // Delete from Vapi if it has an assistant
...
function VoiceAgentCard({
  ...
  return (
    ...
    >
      ...
      {/* Card actions — stopPropagation so clicks don't open the drawer */}
      <div
        ...
        if (token) {
          if (agent.vapi_assistant_id) {
            ...
            if (createRes.ok) {
              ...
              if (newAssistantId) {
                // Write vapi_assistant_id back so future saves do PATCH, not POST
                await supabase
                  .from("voice_agents")
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          ...
          onClick={() => {
            if (!agent.vapi_assistant_id) {
              toast.error("Save the agent first to create a Vapi assistant before testing");
              return;
            }
            setTestOpen(true);
          }}
...
function TestAgentModal({
  ...
  const startCall = async () => {
    ...
    if (!vapi) {
      toast.error(
        "Vapi Web SDK not available. Add VITE_VAPI_PUBLIC_KEY to your .env file."
      );
      return;
    }
      ...
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            Browser call to <span className="font-medium">{agent.name}</span>.
