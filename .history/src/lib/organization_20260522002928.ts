...
async function reloadTeam(orgId: string) {
  console.log("[org] reloadTeam called for", orgId);
  const [{ data: members, error: mErr }, { data: invites, error: iErr }] = await Promise.all([
    supabase.from("org_memberships").select(`
  member_id, role,
  profiles!org_memberships_member_id_fkey(id, first_name, last_name, email)
`).eq("org_id", orgId),
    supabase.from("invitations").select("*")
      .eq("organization_id", orgId).eq("status", "pending"),
  ]);

  console.log("[org] members:", members, mErr);
  console.log("[org] invites:", invites, iErr);
  ;
