...
async function loadOrgFromSupabase() {
  try {
    ...
    // Realtime subscriptions — set up once per session
    if (!orgSubscribed) {
      orgSubscribed = true;
      ...
      // Membership changes → reload team
      ...
      // Invitation changes → reload team
...
export function updateMember(id: string, patch: Partial<TeamMember>) {
  ...
  (async () => {
    try {
      ...
      // 4. Re-fetch team from DB so page shows authoritative values
