...
export function updateMember(id: string, patch: Partial<TeamMember>) {
  ...
  (async () => {
    try {
      ...
      // Single admin function call — handles profiles + org_memberships + auth.users
      ...
      if (!res.ok) {
        console.error("[org] update-user-by-id failed:", await res.text());
      }

      // Small delay for PgBouncer read-after-write consistency, then reload
