...
async function loadOrgFromSupabase() {
  try {
    ...
    if (orgData) {
      // Treat empty-string same as null; discard stale blob: URLs from older code
      ...
      org = {
        ...
        // Use || so an empty-string or null DB value falls through to localStorage cache
      ...
      // Realtime subscription — only subscribe once across all loadOrgFromSupabase calls
      if (!orgSubscribed) {
        orgSubscribed = true;
      supabase
        .channel(`org_realtime_${orgId}`)
        .on("postgres_changes", { event: "UPDATE", schema: "public", table: "organizations", filter: `id=eq.${orgId}` },
          (payload) => {
            const d = payload.new as any;
            const rawLogo = d.logo_url || null;
            const rl = rawLogo?.startsWith("blob:") ? null : rawLogo;
            org = { ...org, companyName: d.name || d.public_name || org.companyName, primaryPhone: d.phone || org.primaryPhone, website: d.website || org.website, industry: d.industry || org.industry, address: d.address || d.business_address || org.address, logoUrl: rl ?? readLogoCache(), crmGoals: d.crm_goals || org.crmGoals, timezone: d.timezone || org.timezone };
    ...
    // Also subscribe to org_memberships changes so team list updates in real-time
supabase
  .channel(`members_${orgId}`)
  .on("postgres_changes",
    { event: "*", schema: "public", table: "org_memberships", filter: `org_id=eq.${orgId}` },
    () => loadOrgFromSupabase()  // re-fetch full team on any membership change
  )
  .subscribe();

supabase
  .channel(`invitations_${orgId}`)
  .on("postgres_changes",
...
team = [...team, ...invitedMembers.filter(m => !existingIds.has(m.id))];
      emitTeam();
    }
  } catch (err) {
