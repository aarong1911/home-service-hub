...
// ── Team reload (without re-subscribing) ──────────────────────────────────────
// Called by realtime callbacks — only refreshes members/invitations data.

async function reloadTeam(orgId: string) {
...
async function loadOrgFromSupabase() {
  try {
    ...
    // Set up realtime subscriptions — only once per session
    if (!orgSubscribed) {
      orgSubscribed = true;

      // 1. Org row changes (logo, name, etc.)
      ...
      // 2. Membership changes → reload team list
      ...
      // 3. Invitation status changes → reload team list
  ...
  (async () => {
    try {
      ...
      if (Object.keys(u).length > 0) {
        ...
        if (error) console.error("[org] update failed:", error);
      }
    } catch (err) {
...
export function updateMember(id: string, patch: Partial<TeamMember>) {
  ...
  (async () => {
    ...
    if (patch.role !== undefined) {
      await supabase.from("org_memberships")
        .update({ role: patch.role })
        .eq("member_id", id);
    }

    // Update phone + name on auth.users row via admin function
    if (patch.phone !== undefined || patch.name !== undefined) {
      ...
      if (session) {
        ...
        // Re-fetch team so page reflects DB values
    const { data: p2 } = await supabase.from("profiles")
      .select("organization_id").eq("id", (await supabase.auth.getSession()).data.session?.user.id ?? "").maybeSingle();
    if (p2?.organization_id) await reloadTeam(p2.organization_id);
