...
export const handler: Handler = async (event) => {
  ...
  let body: { email: string; role: string; name?: string; phone?: string; workerType?: string };
  try { body = JSON.parse(event.body ?? "{}"); }
  catch { return { statusCode: 400, body: JSON.stringify({ error: "Invalid JSON" }) }; }

  const { email, role, name, phone, workerType } = body;
  ...
  try {
    ...
    console.log(`[invite-member] invited ${email} to org ${orgId} as ${role} (${workerType})`);
  } catch (mailErr) {
    console.error("[invite-member] email failed:", mailErr);
    return { statusCode: 207, headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ success: false, invitationId: invitation.id, error: `Invitation saved but email failed: ${(mailErr as Error).message}` }) };
  }
