...
export async function runPostCallAutomation(params: {
  ...
  // Skip if contact was already saved during the call (via save_lead tool)
  ...
  if (extracted.service || extracted.budget || extracted.timeline) {
    ...
    if (leadId) {
      ...
      const dealId = await createPipelineDeal({
        ...
        contactId,
        leadId,
        title: capitalizeService(extracted.service) || 'Inbound Call Lead',
        value: dealValue,
        notes: extracted.summary,
  ...
  if (extracted.appointmentRequested && extracted.appointmentDate && extracted.appointmentTime) {
    const scheduledStr = `${extracted.appointmentDate} ${extracted.appointmentTime}`;
    ...
    if (!isNaN(scheduled.getTime())) {
      const { error: apptErr } = await supabase
        ...
        .insert({
          ...
          service: extracted.service || 'Consultation',
      ...
      log(`could not parse appointment date: "${scheduledStr}"`);
    }
  }
