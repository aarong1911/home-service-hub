...
async function getOrgTimezone(tenantId: string): Promise<string> {
  ...
  console.warn(
    `[vapi-webhook] Could not determine timezone for org ${tenantId}, defaulting to UTC`
  );
  return 'UTC';
}
...
function classifyOutcome(summary = ''): string {
  ...
  if (s.includes('lead') || s.includes('contact info') || s.includes('submitted')) {
    return 'lead_captured';
  }
  if (s.includes('call back') || s.includes('callback') || s.includes('call me back')) {
    return 'callback_requested';
  }
  if (s.includes('not interested') || s.includes('no thank') || s.includes('remove')) {
...
function fireMakeWebhook(url: string | undefined, payload: Record<string, unknown>): void {
  ...
  }).catch(err => logError('fireMakeWebhook', 'post failed', err));
}
...
async function resolveTenant(
  ...
  if (call.assistantId) {
    ...
    if (error) {
      logError('resolveTenant', 'voice_agents lookup', error);
    }
...
async function handleEndOfCallReport(
  ...
  log('end-of-call-report', 'raw call fields', {
    startedAt: call.startedAt,
    endedAt: call.endedAt,
    cost: call.cost,
    status: call.status,
  });
  log('end-of-call-report', 'raw artifact fields', {
    summary: artifact.summary?.slice(0, 100),
    recordingUrl: artifact.recordingUrl,
    messageCount: artifact.messages?.length ?? 0,
  });
  ...
  log('end-of-call-report', `outcome=${outcome} sentiment=${sentiment} duration=${durationSec}s`);
  ...
  if (existing) {
    ...
    if (error) logError('end-of-call-report', 'insert fallback', error);
  }
  ...
  // ── Post-call automation: extract data from transcript → CRM records ──
  // Fire-and-forget so it doesn't block the webhook response.
  // Only runs if save_lead wasn't already called during the call.
  const callRowForAutomation = existing
    ? existing
    : await supabase
        .from('voice_calls')
        .select('id')
...
async function toolBookAppointment(
  ...
  if (!dateStr || !timeStr) {
    return `To book the appointment I need both a date and time. What day and time works for you? Today is ${todayLabel()}.`;
  }

  const scheduled = buildScheduledAt(dateStr, timeStr);
  if (!scheduled) {
    return `I couldn't understand that date or time. Could you say something like "Tuesday at 10am"? Today is ${todayLabel()}.`;
  }

  const { wallClock } = scheduled;
  log('book_appointment', `wallClock=${wallClock}`);
  ...
  log('book_appointment', `tzOffset=${tzOffset} dbScheduledAt=${dbScheduledAt}`);

  const contactId = await upsertContact({ tenantId, name, phone, email, address });
  log('book_appointment', `contactId=${contactId}`);
  ...
  if (apptErr) {
    ...
    return "I had trouble booking in our system, but I've saved all your details. Someone will confirm shortly.";
  }

  log('book_appointment', `appointment created id=${apptData?.id}`);
  ...
  if (contactId) {
    ...
    log('book_appointment', `leadId=${leadId}`);
    ...
    if (leadId) {
      ...
      if (!dealValue) {
        ...
        if (dealValue) log('book_appointment', `dealValue from lead row: ${dealValue}`);
      }

      log('book_appointment', `dealValue=${dealValue}`);
  ...
  supabase
    ...
    .maybeSingle()
    .then(({ data: callRow }) => {
      if (callRow && apptData?.id) {
        supabase
          .from('appointments')
          .update({ voice_call_id: callRow.id })
          .eq('id', apptData.id)
          .then(({ error }) => {
            if (error) logError('book_appointment', 'back-fill voice_call_id', error);
          });
      }
    });

  pushToGoogleCalendar(tenantId, {
  ...
  }).catch(err => logError('book_appointment', 'gcal push', err));
  ...
  return `You're all set, ${firstName}! I've booked your ${service} for ${displayDate} at ${displayTime}. We'll send a confirmation and our team will be ready for you.`;
}
...
async function toolGetServiceInfo(
  ...
  return `We offer ${
    service || 'a full range of home improvement and renovation services'
  } for residential and commercial properties. For accurate pricing, we do a free on-site estimate — I can book one for you right now if you'd like.`;
}
...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  if (eventType === 'end-of-call-report' || eventType === 'call.ended.report') {
    log('handler', 'end-of-call message top-level values', {
      ...
      endedAt: (message as any).endedAt,
      endedReason: (message as any).endedReason,
      cost: (message as any).cost,
      durationSeconds: (message as any).durationSeconds,
      summary: (message as any).summary?.slice?.(0, 80),
    });
  }
  ...
  if (
    ...
    const callerNumber =
      rawMessage?.customer?.number ??
      rawMessage?.call?.customer?.number ??
      null;
    ...
    if (phoneErr || !phoneRow) {
      logError('assistant-request', 'phone number lookup failed', {
        phoneErr,
        phoneNumberId,
      });
      return {
    ...
    if (phoneRow.agent_id) {
      ...
      if (mappedErr) {
        logError('assistant-request', 'mapped agent lookup failed', mappedErr);
      }
    ...
    if (activeErr) {
      logError('assistant-request', 'active agent lookup failed', activeErr);
    }
  ...
  switch (eventType) {
    ...
    case 'call.ended.report': {
      const mergedCall: VapiCall = {
        ...call,
        startedAt: call?.startedAt ?? (message as any).startedAt,
        endedAt: call?.endedAt ?? (message as any).endedAt,
        endedReason: call?.endedReason ?? (message as any).endedReason,
        cost: call?.cost ?? (message as any).cost,
      };

      const mergedArtifact: VapiArtifact = {
        ...(message.artifact ?? {}),
        summary: message.artifact?.summary ?? (message as any).summary ?? '',
      };
      ...
      await handleEndOfCallReport(mergedCall, mergedArtifact, tenantId, agentId);
      break;
    }
