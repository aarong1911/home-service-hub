...
export type TeamMembersManagerProps = {
  ...
  /** When true, defaults the add dialog to "invited" state (used in onboarding). */
  inviteMode?: boolean;
  title?: string;
  subtitle?: string;
};
...
function MemberDialog({
  ...
  function handleSubmit() {
    ...
    const status: TeamMember["status"] =
      mode === "add" && (inviteDefault || sendNow) ? "invited" : initial?.status ?? "active";
    onSave({
      name: name.trim(),
        ...
        <div className="grid gap-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              ...
              <Input className="mt-1.5 h-9" value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name" />
            </div>
            <div>
              <Label className="text-xs">Email</Label>
              <Input className="mt-1.5 h-9" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@company.com" />
            </div>
            <div>
